...
const register = asyncHandler(async (req, res) => {
    ...
    if (!name || !password || !email) {
        res.status(201).json({ message: "Please provide all the deatils!" });
    }
    ...
    try {
        ...
        res.status(201).json({ message: "Error while createing user", error: error })
    }
});
...
const login = asyncHandler(async (req, res) => {
    ...
    if (!email || !password) {
       return  res.status(201).json({ message: "Please fill all the field!" })
    }
